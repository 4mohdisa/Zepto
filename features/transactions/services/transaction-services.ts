...
import { 
  ...
  getNextDates 
} from '@/lib/utils/frequency-utils'
import {
...
export class TransactionService {
   ...
   * Uses recurring_transaction_id to prevent duplicate creation
   */
  async generateDueTransactions(
    ...
    try {
      ...
      for (const rt of transactions) {
        ...
        // Get the most recent due date (could be today or earlier)
        ...
        // Insert with conflict handling - database unique constraint will prevent duplicates
        // The unique index on (user_id, recurring_transaction_id, date) ensures idempotency
        ...
        if (insertError) {
          ...
          // This can happen if two requests race or if React Strict Mode double-triggers
          const isDuplicateError = insertError.code === '23505' || // PostgreSQL unique_violation
            insertError.message?.includes('duplicate') ||
            insertError.message?.includes('unique constraint')
          
          if (isDuplicateError) {
            // This is expected if transaction already exists - silently skip
            continue
          }
          
          // Log other errors but continue processing other recurring items
